...
def optimize2(p_pv, p_consumed, p_ut, dt=1, size_elh=None, size_bess=None, size_hss=None, vol_hss_water=None, run_lp=False,
             **kwargs):
    """
    Function to optimization energy flows for 2 users, 2 PV, 2 ELH and 1 BESS
    """
    ...
    pv_flag = [user_name in p_pv.columns for u, user_name in enumerate(p_pv)]
    cl_flag = [user_name in p_ut.columns for u, user_name in enumerate(p_ut)]
    ue_flag = [user_name in p_consumed.columns for u, user_name in enumerate(p_consumed)]
    #pv_flag = [(p_pv[col].notna() & (p_pv[col] != 0)).any() for col in p_pv.columns]
    ...
    M_grid_in = 1.1 * (p_pv['pv1']+p_pv['pv2']).max()  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if bess_flag:
        M_grid_in += size_bess / t_bess_min
    # fake-big-M parameter for maximum grid withdrawals
    M_grid_out = 1.1 * (p_consumed['consumer1']+p_consumed['consumer2']).max()
    M_cl_rec = (p_pv['pv1']+p_pv['pv2']).max()
    if bess_flag:
    ...
    if cl_flag:
        ...
        M_grid_out += (p_ut['thermal_user1']+p_ut['thermal_user2']).max() * 1.1
    ...
    user_ids = [0, 1]
    p_cl_with = np.array(
        [[pulp.LpVariable(f'Pcl_with_{u}_{t}', lowBound=0) if cl_flag[u] else 0 for t in time_set] for u in user_ids])

    # controlled load input from the grid
    p_cl_grid = np.array(
        [[pulp.LpVariable(f'Pcl_grid_{u}_{t}', lowBound=0) if cl_flag[u] else 0 for t in time_set] for u, _ in
